namespace Business.Repositories.UserRepository.Constants
{
    public class UserMessages
    {
        public static string Added = "User added successfully.";
        public static string Deleted = "User deleted successfully.";
        public static string Updated = "User updated successfully.";
        public static string Retrieved = "User retrieved successfully.";
        public static string Listed = "Users listed successfully.";
        public static string NotFound = "User not found.";
        public static string WrongPassword = "Wrong password.";
        public static string PasswordChanged = "Password changed successfully.";
    }
}
